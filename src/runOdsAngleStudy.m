...
fprintf("Selected outer angle: %.2f deg\n", results.selected.outerAngleDeg);
fprintf("Selected inner angle: %.2f deg\n", results.selected.innerAngleDeg);
fprintf("Strict worst-case exceedance: %.3f %% (limit %.3f %%)\n", ...
    results.selected.worstExceedancePercent, ...
    cfg.reporting.strictAllowedExceedancePercent);
fprintf("Practical outer/inner: %.2f/%.2f deg; robust availability %.3f %%\n", ...
